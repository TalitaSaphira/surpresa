...
//  Serial.write((uint8_t *)covariance, sizeof(covariance));
  ...
  if(Serial.available() > 0){
    ...
    int valorPWMVelLB = map(velLB, 0.0, 1.0, 0, 255);
    int valorPWMVelRB = map(velRB, 0.0, 1.0, 0, 255);
    int valorPWMVelLF = map(velLF, 0.0, 1.0, 0, 255);
    int valorPWMVelRF = map(velRF, 0.0, 1.0, 0, 255);

    // Generate PWM signal for motor steps (adjust the delay for speed control)
    analogWrite(stepLB, valorPWMVelLB);
    analogWrite(stepRB, valorPWMVelRB);
    analogWrite(stepLF, valorPWMVelLF);
    analogWrite(stepRF, valorPWMVelRF);
